...
import codeql.php.dataflow.CallGraph

private int countMethodCalls() { result = count(MethodCall mc) }

private int countTypedMethodCalls() { result = count(MethodCall mc | hasResolvedReceiver(mc)) }

private int countStaticCalls() { result = count(StaticMethodCall sc) }

private int countTypedStaticCalls() {
  result = count(StaticMethodCall sc | hasResolvedStaticTarget(sc))
}
