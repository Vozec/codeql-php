...
## Status: 197 / 197 annotated sinks detected, 0 false positives
...
- **WP `apply_filters($tag, $value, …)` carries taint arg1→return** — the central WP value-passing idiom: every plugin accessor/option read funnels a value through `apply_filters`, which returns it (filtered, or unchanged when no hook is attached). Left unmodeled it silently dropped taint mid-flow, hiding any request→accessor→sink path that crosses a filter (Web Directory Free CVE-2024-3552, whose `w2dc_getValue()` accessor pipes every request value through it). Benchmark FP unchanged at 40/176.
